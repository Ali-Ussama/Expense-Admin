package com.expense.expenseadmin.Utilities;

public class PlaceColumns {
    public static String id = "id";
    public static String name = "name";
    public static String description = "description";
    public static String phoneNumber = "phoneNumber";
    public static String facebookUrl = "facebookUrl";
    public static String twitterUrl = "twitterUrl";
    public static String websiteUrl = "websiteUrl";
    public static String likesCount = "likesCount";
    public static String okayCount = "okayCount";
    public static String dislikesCount = "dislikesCount";
    public static String locationModels = "locationModels";
    public static String imagesURL = "imagesURL";
    public static String placeID = "placeID";
    public static String url = "url";
    public static String country = "country";
    public static String city = "city";
    public static String street = "street";
    public static String latitude = "latitude";
    public static String longitude = "longitude";
    public static String category = "category";
}
